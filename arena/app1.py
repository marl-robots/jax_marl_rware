...
def _band_fig(
    ...
    canon = re.compile(
        r"^(ia2c|ippo|maa2c|mappo|seac)_" r"(tiny|small|medium|large)-\d+ag$"
    )
    ...
    for r in pool:
        ...
        if prev is None or float(r.df["environment_steps"].max()) > float(
            prev.df["environment_steps"].max()
        ):
            groups[stem][seed] = r  # dedupe (stem,seed): keep the longer run
    fig = go.Figure()
    order_key = lambda s: (
        (
            ["ia2c", "ippo", "maa2c", "mappo", "seac"].index(s.split("_")[0])
            if s.split("_")[0] in ("ia2c", "ippo", "maa2c", "mappo", "seac")
            else 9
        ),
        s,
    )
    for stem in sorted(groups, key=order_key):
        ...
        for r in frs:
            ...
            y = (
                r.df[col].ewm(span=smooth, min_periods=1).mean().to_numpy()
                if smooth > 1
                else r.df[col].to_numpy(dtype=float)
            )
            order = np.argsort(x)
...
aggregate = st.sidebar.checkbox(
    "aggregate seeds → mean±std bands",
    value=_has_multiseed,
...
st.sidebar.caption(
    f"{len(all_runs)} runs discovered"
    + (f" · {max(_fam_seed_counts.values())} seeds max/algo" if _has_multiseed else "")
)
    ...
    for r in runs_full_met:
        ...
        for group, metrics in new_full_metrics.items():
            ...
            for i, (col, label) in enumerate(metrics.items()):
                figs_list.append(
                    auto_plot(r, label, col, smooth=smooth, show_raw=show_raw)
                )
...
with tab_overview:
    ...
    if aggregate:
        st.caption(
            "📊 **aggregated**: each curve is the across-seed mean with a "
            "±1 std band — toggle off in the sidebar for per-run lines."
        )
    st.plotly_chart(
        _series_fig("episode_returns_mean", "The race — team return"),
        width="stretch",
    )
    #st.plotly_chart(
    #    _curve_fig(
...
with tab_compare:
    if aggregate:
        st.caption(
            "Showing across-seed **mean ± 1 std** per algorithm "
    ...
    for group, metrics in METRIC_GROUPS.items():
        ...
        for i, (col, label) in enumerate(metrics.items()):
            with gcols[i % len(gcols)]:
                st.plotly_chart(_series_fig(col, label), width="stretch",)
                #st.plotly_chart(
