...
from algorithms.replay import make_seac_recorder, save_replay
...
def main():
    ...
    if args.resume:
        run_dir = args.run_dir or os.path.join(
            ...
            "resume",
            f"{date_time}",
            f"seac{net_tag}_Warehouse_{cfg.size}-{cfg.n_agents}ag_seed{cfg.seed}",
        )
    else:
        run_dir = args.run_dir or os.path.join(
            "runs",
    ...
    print(
        f"algo=seac (per-agent A2C, seac_coef={cfg.seac_coef}, "
    ...
    recorder = (
        make_seac_recorder(trainer["env"], trainer["actor"], cfg)
        if replay_every > 0
        else None
    )
    ...
    def record_snapshot(upd_now: int) -> None:
        ...
        if recorder is not None:
            traj, scalars = recorder(carry[0], replay_key)
            path = save_replay(
                run_dir,
                traj,
                scalars,
    ...
    while upd < n_updates:
        ...
        if use_full_metrics and block_processor is not None:
            expended_raw = {}
            for key, v in metrics_tensors.items():
                # tuple to dict

                if "grads" in key:
                    expended_raw[key] = {"actor": v[0], "critic": v[1]}
                # (U, T, E, N)->(U, 1, T, E, N)
                # (U,)->(U, 1)

                elif "epoch" in key and isinstance(v, jax.numpy.ndarray):
                    if (
                        not "epoch_loss" in key
                        or "epoch_old_logp" in key
                        or "epoch_grads" in key
                    ):
                        expended_raw[key] = jax.numpy.expand_dims(
                            metrics_tensors[key], axis=1
                        )
                    elif "epoch_loss" in key:
                        expended_raw[key] = jax.numpy.expand_dims(
                            metrics_tensors[key], axis=-1
                        )
                    else:
                        expended_raw[key] = metrics_tensors[key]
                else:
                    expended_raw[key] = metrics_tensors[key]

            metrics_tensors = expended_raw
