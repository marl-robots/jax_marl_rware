...
def iql_update(qnet: QNetwork, tx, cfg, params, target_params, opt_state,
               rew_ms, batch):
    ...
    if cfg.double_q:
        cur_max_actions = jnp.argmax(
            jax.lax.stop_gradient(mac_out)[1:], axis=-1)     # [T-1, E, N]
        target_max = jnp.take_along_axis(
            target_next, cur_max_actions[..., None], axis=-1)[..., 0]
    else:
    ...
    def loss_fn(p):
        ...
        chosen = jnp.take_along_axis(
            q[: T - 1], actions_T[: T - 1][..., None], axis=-1)[..., 0]  # [T-1,E,N]
...
def make_iql_trainer(cfg):
    ...
    def init_state(key):
        ...
        params = qnet.init(
            ki, ScannedGRU.initialize_carry(B, H),
    ...
    def collect(params, key, epsilon):
        ...
        obs_seq = jnp.concatenate(
            [obs_t, final_obs.reshape(1, B, in_dim)], axis=0)     # [T+1, B, in]
        metrics = {
    ...
    return dict(env=env, qnet=qnet, tx=tx, N=N, E=E, T=T, in_dim=in_dim,
...
def init_ensemble(qnet, key, K, B, H, in_dim):
    ...
    def init_one(k):
        return qnet.init(k, ScannedGRU.initialize_carry(B, H),
                         (jnp.zeros((1, B, in_dim)), jnp.zeros((1, B))))
...
def emax_update(qnet, tx, cfg, params_ens, opt_state, rew_ms, batch):
    ...
    return params_ens, opt_state, rew_ms, {"loss": loss, "q_mean": q_mean}
...
def make_emax_trainer(cfg):
    ...
    def collect(params_ens, key, epsilon):
        ...
        metrics = {"deliveries": deliv_t.sum(0).sum(-1).mean(),
    ...
    return dict(env=env, qnet=qnet, N=N, E=E, T=T, in_dim=in_dim, K=K,
                init_state=init_state, collect=collect, update=update)
