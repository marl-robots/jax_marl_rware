...
class Narrator:
    ...
    def chunk(self, update: int, stats: dict) -> str:
        d = stats["mean_deliveries"]
        b = stats["mean_block_rate"]
        idle = stats["mean_idle_rate"]
        ret = stats["mean_episode_return"]
        e, m, l = (stats["mean_deliveries_early"], stats["mean_deliveries_mid"],
                   stats["mean_deliveries_late"])

        base = self.base or {}
        d_arrow = _arrow(d, base.get("mean_deliveries"))
        b_arrow = _arrow(b, base.get("mean_block_rate"))
        ret_arrow = _arrow(ret, base.get("mean_episode_return"))
        ...
        new_base = dict(base)
        for k in ("mean_deliveries", "mean_block_rate", "mean_idle_rate", "mean_episode_return"):
