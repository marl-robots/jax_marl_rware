...
class Conv1DEncoder(nn.Module):
    hidden_dim: int
    num_layers: int = 2  # כמה שכבות Conv1D
    features: int = 64  # כמה פילטרים בכל שכבה
    ...
    def __call__(self, x):
        ...
        for _ in range(self.num_layers):
            x = nn.Conv(
                features=self.features, kernel_size=(3,), strides=(1,), padding="SAME"
            )(x)
            x = nn.relu(x)

        # Mix all features globally
        x = nn.Dense(self.hidden_dim)(x)
        return x


class ActorCNN_RNN(nn.Module):
    num_actions: int
    hidden_dim: int = 128
    ...
    def __call__(self, hidden, x):
        obs, dones = x  # obs: [T, B, obs_dim]

        # --- Conv1D stack BEFORE Dense ---
        embedCNN = Conv1DEncoder(
        ...
        # --- GRU ---
        if self.use_rnn:
            embed = nn.relu(nn.Dense(self.hidden_dim)(embedCNN))
